...
begin;
do $$
declare tid uuid:='11111111-1111-4111-8111-111111111111'; c record; s public.services%rowtype; target_day date; start_at timestamptz; finish_at timestamptz; blocked timestamptz; staff uuid; aid uuid; j integer; attempt integer; ordinal integer:=0;
begin
 ...
 for c in select id from public.customers where tenant_id=tid and id::text like '60000000-0000-4000-8000-%' order by id limit 8 loop
  ordinal:=ordinal+1;
  staff:=('30000000-0000-4000-8000-'||lpad(((ordinal-1)%4+1)::text,12,'0'))::uuid;
  for j in 1..4 loop
   aid:=('71000000-0000-4000-8000-'||lpad((ordinal*10+j)::text,12,'0'))::uuid;
   ...
   target_day:=(now() at time zone 'Europe/Lisbon')::date-(j*28+ordinal);
   ...
   select aid,tid,l.id,c.id,s.id,staff,s.name,s.price_minor,start_at,finish_at,blocked,
   case when ordinal%3=0 and j=1 then 'cancelled' when ordinal%4=0 and j=2 then 'no_show' else 'completed' end
   ...
   update public.customers set created_at=least(created_at,start_at-interval '1 day') where id=c.id and tenant_id=tid;
  end loop;
 end loop;
end $$;
commit;
