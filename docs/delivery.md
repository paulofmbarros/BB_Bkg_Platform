...
The Supabase management credential is used for migrations and deploying/configuring the owner invitation Edge Function. It is not sent to Vercel or loaded into the application build. Vercel settings are checked against the selected database and workspace before migrations. The Next.js hosted runtime still uses only the publishable Supabase key and caller sessions. The separate invitation function uses Supabase's built-in privileged credentials after verifying the caller's platform role; see `platform-administration.md` for required Auth redirect and email settings.
